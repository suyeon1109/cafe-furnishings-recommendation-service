...
'''
"optDescription": "규격: 820 x 518 x 552\n무게(Kg): 63\n용량(L): 11\n전기 사양: 4100W\n색상: 스틸\n소재: 스틸",
'''

# def weight(category):

#     string.split(maxsplit=1)
#     espresso_weight_max = db.item.find({"optDescription": {"$lte":bud*plan_weight["에스프레소머신"][1]/100}}).sort("price", pymongo.DESCENDING)

# espresso machine recommendation
espresso_max = db.item.find({
    "category": "에스프레소머신", 
    "price": {"$lte":bud*plan_price["에스프레소머신"][1]/100}
}).sort("price", pymongo.DESCENDING).limit(5)

for doc in espresso_max:
    weight_str = doc["optDescription"].split(": ",maxsplit=1)
    print(weight_str)
